...
把战斗特质从 **名字子串解锁** 收成 **`defId` 解锁**；本 feat **只做已定义的三特质**（英雄 / 医生 / 小贼）并补齐边界与 EditMode。  
**不做**新人物特质、不做牌库化。

**验收说明：** 医生/小贼依赖事件入住或 Debug TakeIn；与 EVT-F03 人设对齐后完整 Play。
...
| 4 | 三特质行为可测：英雄 ManualOnce；医生 RoundEnd Heal；小贼 TurnStart 伤+偷意图 |
...
- [x] 默认 roster → 仅英雄
- [x] TakeIn doctor/thief → 对应特质；Expel → 失去
- [x] 英雄 once；医生 RoundEnd；小贼伤害+偷

### Play（半开放）

- [ ] Debug TakeIn 医生/小贼后战斗槽点亮（手测可选）
- [ ] 正式入住线完整测 → EVT-F03
...
| 2026-08-16 | `nurse`→`doctor`（EVT-F03） |
