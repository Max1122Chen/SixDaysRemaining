...
| `COMB-F01` | 轻量 ASC（CombatComponentBase + AttributeSet） | COMB | Review | `designs/COMB-F01-combat-component-base.md` | — | `feat/combat` | Max | 代码已落；待 Unity Edit Mode |
| `COMB-F02` | 战斗 AttributeSet + 伤害/格挡管线 | COMB | Review | `designs/COMB-F02-combat-pipeline.md` | — | `feat/combat` | Max | 同分支 |
| `COMB-F03` | PlayerCombat + 选 5 Commit | COMB | Review | `designs/COMB-F03-player-cards.md` | — | `feat/combat` | Max | 同分支 |
| `COMB-F04` | EnemyCombat + 行为表 + 轻量 Session | COMB | Review | `designs/COMB-F04-enemy-pattern.md` | — | `feat/combat` | Max | 同分支 |
| `COMB-F05` | CombatManager + 编排 + 结算 | COMB | Review | `designs/COMB-F05-combat-manager.md` | — | `feat/combat` | Max | 同分支；Flee；FoodGained int |
